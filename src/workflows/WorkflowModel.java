        ...
        public LinkedList<workflow_properties> getIfExecution(workflow_object obj, boolean condition) {
            ...
            if (condition) {
                ...
                for (int i=0; i<get_nb_workflow_connector_edge();i++) {
                    ...
                    if (edge.source.parent==obj&&edge.source.number==workflow_object.UP) {
                        list_of_objects.add(edge.dest.parent);
                    }
                }
            } else {
                //--Get the first(s) child
                for (int i=0; i<get_nb_workflow_connector_edge();i++) {
                    workflow_connector_edge edge=work_connection.get(i);
                    if (edge.source.parent==obj&&edge.source.number==workflow_object.DOWN) {
                        list_of_objects.add(edge.dest.parent);
                    }
                }
            }
...
//    popMatrix();
    ...
    /**
     * This represent a Begin and End object
     * used to represent Begin and End of an object
     */
