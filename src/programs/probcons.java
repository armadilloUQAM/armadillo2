...
public class probcons extends RunProgram {
    ...
    public probcons(workflow_properties properties) {        
       super(properties);        
        execute();
    }
    ...
    public boolean init_checkRequirements() {
        ...
        if (multiplesequences_id==0) {
         ...
         if (!multi.isAA()) {
             setStatus(status_running,"***Warning. ProbCons required protein sequences. ***\nCurrent sequences are not set as Amino Acid.");
            return true;
         }
        return true;
    }
