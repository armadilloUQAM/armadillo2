 ...
 * access the database)
 * Note: this version is for the local database access (outside of the Armadillo
 * Workflow Platform)
 * @author Etienne Lord, Leclercq Mickael
 * @since June 2009
 * @version 2010
 */
public class databaseFunction_local {
    ...
    public database_local db;                    //application specific database
    public SimpleDateFormat dateformat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    public static boolean debug = false;
    public static boolean initialized=false;    //Flag for initialisation of the clas to have only one instance
    ...
    public databaseFunction_local() {
        ...
        if (!initialized) {
           initialized=true;
           db=new database_local(config.databasePath());
           if (db==null) {
...
//               System.out.println("Will exit. Please delete your config.dat file.");
    ...
    public boolean New(String dbfilename) {
        try {
            ...
            boolean TableCreated=createTables();
    ...
    public boolean New(String dbfilename, String sqlfilename) {
        try {
            ...
            db.connect();
            //--Create the new table (SQL statement)
            boolean TableCreated=createTables(sqlfilename);
            //--Copy the project informations
            if (TableCreated) {
                project.saveToDatabase();
            }
...
//    }
     ...
     * @return
     */
    public database_local getDatabase() {
        return db;
    }   
